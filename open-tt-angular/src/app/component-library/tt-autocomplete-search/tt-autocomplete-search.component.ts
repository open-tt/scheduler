...
export class TtAutocompleteSearchComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.filteredPlayerNames = this.myControl.valueChanges
      .pipe(
        startWith(''),
  ...
  private _filter(name: string): Player[] {
    ...
    const players = this.allPlayerNames.filter(option => option.name.toLowerCase().includes(filterValue));
    if (players.length === 0) {
      this.noMatch = true;
    }
    return players;
  ...
  public addPlayerButtonClick(): void {
    ...
    if (this.player && this.searchString.toLowerCase() === this.player.name.toLowerCase()) {
